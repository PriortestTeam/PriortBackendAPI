...
public class JwtUserServiceImpl implements UserDetailsService {
    ...
    /**
     * Get the associated project ID for the user.
     * This method assumes that the user object has access to the associated project.
     *
     * @param user the user for whom the project ID is being retrieved
     * @return the associated project ID
     */
    private String getUserProjectId(AuthLoginUser user) {
        // Example logic to fetch project ID (you may need to modify this based on your database or user data model)
        // Assuming the user object has a method getUserUseOpenProject() that gives the associated project data.
        if (user != null && user.getSysUser() != null && user.getSysUser().getUserUseOpenProject() != null) {
            return user.getSysUser().getUserUseOpenProject().getProjectId();
        }
        return null;
    }
    ...
    public String saveUserLoginInfo(AuthLoginUser user) {
        ...
        // get Project Id
        String projectId = getUserProjectId(user);
        if (projectId == null) {
        ...
        // 创建token，这个是用户信息
        String sign = JWT.create().withSubject(user.getUsername()).withClaim("projectId", projectId)  // Add project-specific claim
            .withExpiresAt(date).withIssuedAt(new Date()).sign(algorithm);
        String s = JSONObject.toJSONString(user);
        RBucket<String> bucket = redisClient.getBucket(OneConstant.REDIS_KEY_PREFIX.LOGIN + user.getUsername() + ":" + projectId);  // Include projectId in Redis key
        if (bucket.isExists()) {
            bucket.delete();
        }
        bucket.set(s);
        ...
        bucket = redisClient.getBucket(REDIS_KEY_PREFIX.LOGIN_JWT + user.getUsername() + ":" + projectId);  // Include projectId in Redis key
        bucket.set(sign);
    ...
    public void saveUserLoginInfo2(SysUser sysUser) {
        ...
        String redisKey = OneConstant.REDIS_KEY_PREFIX.LOGIN + sysUser.getEmail() + ":" + projectId;

        RBucket<String> bucket = redisClient.getBucket(redisKey);
        if (bucket.isExists()) {
            bucket.delete();  // Delete the existing bucket if it already exists
        }
        bucket.set(userJson);
        bucket.expire(1, TimeUnit.HOURS);
    }
    ...
    private void checkUserExpireDate(final SysUser user) {
        ...
        if (Objects.nonNull(expireDate) && expireDate.before(now)) {
            throw new BizException(SysConstantEnum.USER_EXPIRE_DATE.getCode(), SysConstantEnum.USER_EXPIRE_DATE.getValue(), HttpStatus.BAD_REQUEST.value());
        }
    }
